...
class CustomDataset(Dataset):
    def __init__(self, model,image_list, path, img_size=224, crop=None, is_train=None):
        ...
        self.augm1=CDDataAugmentation1(img_size=224,
                                           ...
                                           long_mask=True)
        # 加载分类标签表
        label_path = r"../BUSI/newtrain/train/train_classification_label.xlsx"
...
def cross_validation(args):
    train_folder = '../BUSI/newtrain/train'
    val_folder = train_folder

    # 读取标签表格
    label_df = pd.read_excel(r"../BUSI/newtrain/train/train_classification_label.xlsx")
    ...
    device = torch.device('cuda')
    import shutil
    save_path = os.path.join("checkpoint1", "Zhou")
    ...
    checkpoint = torch.load("../model/best_model_epoch173_se_only.pt", map_location=device)
