...
class CustomDataset(Dataset):
    def __init__(self, image_list, path, img_size=224, crop=None, is_train=None):
        ...
        if is_train:
            self.augm = CDDataAugmentation(img_size=img_size,
                                           ...
                                           p_scale=0.6,
                                           p_gaussn=0.5, p_contr=0.0, p_gama=0.5, p_distor=0.0,
...
def val(model,val_loader, loss_se,device):
    model.eval()
    running_loss = 0.0
    seg_total_loss = 0.0
    cl_total_loss = 0.0
...
def train(model,optimizer, train_loader,loss_se,device,epoch,n=2):
...
def cross_validation(args):
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        train_loss,train_seg_loss = train(model,optimizer, train_loader,se_loss,device, epoch)
        val_loss, dice_score,hd_score,iou_score,val_seg_loss = val(model,val_loader,se_loss,device)
