...
def inference_all(model, test_loader, device, save_dir="test_results1"):
    model.eval()
...
# 数据加载
test_dataset = TestImageDataset(image_dir='BUSI/test_img/test_img')

test_loader = DataLoader(test_dataset, batch_size=16, shuffle=False, num_workers=0)

# 加载模型
model = UNetTaskAligWeight(n_channels=3,n_classes=1).to(device)
checkpoint = torch.load("trained_model/checkpoint/Zhou/best_model_epoch173_se_only.pt", map_location=device)
